...
public class StoerWagnerMinimumCut<V, E>
{
	...
	protected void minimumCutPhase(Set<V> a)
	{
		...
		for (Set<V> v : workingGraph.vertexSet ())
		{
			if (v != a)
			{
				Double w = -workingGraph.getEdgeWeight (workingGraph.getEdge (v, a));
		...
		List<Set<V>> list = new ArrayList<Set<V>> (workingGraph.vertexSet ().size ());
		list.add (a);
		while ( !queue.isEmpty ())
		{
			...
			for (DefaultWeightedEdge e : workingGraph.edgesOf (v))
			{
				...
				if (dmap.get (vc) != null)
				{
					Double neww = -workingGraph.getEdgeWeight (workingGraph.getEdge (v, vc)) + dmap.get (vc).weight;
