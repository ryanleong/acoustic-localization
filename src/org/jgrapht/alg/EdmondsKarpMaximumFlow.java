 ...
 * "http://en.wikipedia.org/wiki/Edmonds-Karp_algorithm">Edmonds-Karp algorithm</a>. Be careful: for
 * large networks this algorithm may consume significant amount of time (its upper-bound complexity
 * is O(VE^2), where V - amount of vertices, E - amount of edges in the network).
 * 
 * <p>
...
public final class EdmondsKarpMaximumFlow<V, E>
{
	...
	private double				epsilon;						// tolerance (DEFAULT_EPSILON or
																// user-defined)
	...
	private Map<V, Integer>		indexer;						// mapping from vertices to their
																// indexes
																// in the internal representation
	private List<Node>			nodes;							// internal representation of the
																// network
	 ...
	 * capacities are weights, otherwise all
			...
			if ( !nodes.get (currentSink).visited)
			{
				...
				for (int i = 0; i < numNodes; i++)
				{
					for (Arc currentArc : nodes.get (i).outgoingArcs)
					{
						...
						{
							maximumFlow.put (currentArc.prototype, currentArc.flow);
						}
					}
				}
				return;
			...
			for (Arc currentArc : nodes.get (currentNode).outgoingArcs)
			{
				if ((currentArc.flow + epsilon) < currentArc.capacity)
				{
					if ( !nodes.get (currentArc.head).visited)
					{
						...
						nodes.get (currentArc.head).flowAmount = Math.min (nodes.get (currentNode).flowAmount, currentArc.capacity - currentArc.flow);
	...
	class Node
	{
		V			prototype;									// corresponding node in the
																// original network
		...
		double		flowAmount;								// amount of flow, we are able to
																// push here
