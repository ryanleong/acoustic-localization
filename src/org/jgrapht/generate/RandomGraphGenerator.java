...
public class RandomGraphGenerator<V, E> implements GraphGenerator<V, E, V>
{
	 ...
	 *             isNumberOfEdgesValid(org.jgrapht.Graph,
	 *             int)
	 * 
	...
	public void generateGraph(Graph<V, E> target, VertexFactory<V> vertexFactory, Map<String, V> resultMap)
	{
		...
		{
			throw new IllegalArgumentException ("numOfEdges is not valid for the graph type " + "\n-> Invalid number Of Edges=" + numOfEdges
					+ " for:" + " graph type=" + target.getClass () + " ,number Of Vertexes=" + this.numOfVertexes
	...
	public interface EdgeTopologyFactory<VV, EE>
	{
		 ...
		 * vertexes in graph <li>a Simple Graph, can have max of #V*(#V-1)/2 edges. etc
		 * 
		 * @param targetGraph guranteed to start with zero edges.
		 * @param numberOfEdges
		 */
	 ...
	 * choose another and try again. Performance: <li>when the number of possible edges becomes slim
	 * , this class will have a very poor performance , cause it will not use gready methods to
	 * choose them. for example : In simple graph , if #V = N (#x = number Of x) and we want full
	 * mesh #edges= N*(N-1)/2 , the first added edges will do so quickly (O(1) , the last will take
	 * O(N^2). So , do not use it in this kind of graphs. <li>If the numberOfEdges is bigger than
	 * what the graph can add, there will be an infinite loop here. It is not tested.
	 * 
	 * @author Assaf
	 * @since Aug 6, 2005
	 */
	public class DefaultEdgeTopologyFactory<VV, EE> implements EdgeTopologyFactory<VV, EE>
	{
		public void createEdges(Graph<VV, EE> targetGraph, Map<Integer, VV> orderToVertexMap, int numberOfEdges, Random randomizer)
		{
			...
			while (edgesCounter < numberOfEdges)
			{
				...
				VV startVertex = orderToVertexMap.get (Integer.valueOf (randomizer.nextInt (numOfVertexes)));
				VV endVertex = orderToVertexMap.get (Integer.valueOf (randomizer.nextInt (numOfVertexes)));
				try
				{
		 ...
		 * </table>
		 * 
		 * @see RandomGraphGenerator.EdgeTopologyFactory#isNumberOfEdgesValid(Graph, int)
		 */
		public boolean isNumberOfEdgesValid(Graph<VV, EE> targetGraph, int numberOfEdges)
		{
			boolean result;
