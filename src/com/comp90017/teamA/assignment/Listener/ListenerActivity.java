...
public class ListenerActivity extends Activity implements SensorEventListener,
	...
	public static final String defaultSessionID = "grkperjop";
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		EditText sessionIDEditText = (EditText) findViewById(R.id.sessionIDTB);
		sessionIDEditText.setText(defaultSessionID);
	}
	...
	public void onClick(View v) {
		...
		if (sessionIDEditText.getText().toString().trim().length() > 0) {
			this.sessionID = sessionIDEditText.getText().toString();
		}
		else {
		...
		case R.id.setupQueryBtn:
			Toast.makeText(getApplicationContext(),
					"Querying for landmark data.", Toast.LENGTH_LONG).show();
		...
		case R.id.TrackEmitterBtn:
			Toast.makeText(getApplicationContext(), "Tracking Emitter...",
					Toast.LENGTH_LONG).show();
			trackEmitter();
			break;
		...
		case R.id.queryBtn:
			Toast.makeText(getApplicationContext(),
					"Querying for emitter data", Toast.LENGTH_LONG).show();
			query();
			break;
			...
			public void doTask(Object... arg0) {
				SoundMeter sm = new SoundMeter() {
					...
					protected void onPostExecute(String result) {
						...
						sensorDistances.add(new DistanceEstimator()
								.calculateDistance(maxDB, 1000));
	...
	private void queueEmit(int startTime) {
		...
		t.addTask(startTime * 1000, new PulseTimeredTask(generatedSnd,
				sampleRate));
	...
	private void queueUpload(int startTime) {
		t.addTask(startTime * 1000, new ScheduledTask() {
			...
			public void doTask(Object... arg0) {
				...
				dataUploadString += System.currentTimeMillis() + ","
						+ sessionID;

				Log.d("Setup Data Send, ID " + listenerID, dataUploadString);
				Toast.makeText(getApplicationContext(), dataUploadString,
						Toast.LENGTH_LONG).show();
			...
			public void doTask(Object... arg0) {
				...
				final ReceiveData receiveData = new ReceiveData() {
					...
					protected void onPostExecute(String result) {
						...
						Log.d("000", id1l1l2 + " " + id1l1l3 + " " + id2l2l1
								+ " " + id2l2l3 + id3l3l1 + " " + id3l3l2);
						...
						gv.addLandmarkEdge(GraphView.landmark1,
								GraphView.landmark2, l1l2);
						gv.addLandmarkEdge(GraphView.landmark2,
								GraphView.landmark3, l2l3);
						gv.addLandmarkEdge(GraphView.landmark3,
								GraphView.landmark1, l1l3);
	...
	public void trackEmitter() {
		...
		tt.addTask(listeningDuration, new ScheduledTask() {
			...
			public void doTask(Object... arg0) {
				...
				Toast.makeText(getApplicationContext(),
						"Estimated distance: " + distance, Toast.LENGTH_SHORT)
						.show();
			...
			protected void onPostExecute(String result) {
				...
				for (twitter4j.Status status : this.feed.getTweets()) {
					...
					if (feedData[1].equals("1")) {
						gv.addEmitterEdge(GraphView.landmark1, Double.parseDouble(feedData[2]));
						
						Log.d("Update Emitter Edge", "Updated edge at " + GraphView.landmark1);
						
					} else if (feedData[1].equals("2")) {
						gv.addEmitterEdge(GraphView.landmark2, Double.parseDouble(feedData[2]));
						
						Log.d("Update Emitter Edge", "Updated edge at " + GraphView.landmark2);
					} else {
						
						gv.addEmitterEdge(GraphView.landmark3, Double.parseDouble(feedData[2]));
						
						Log.d("Update Emitter Edge", "Updated edge at " + GraphView.landmark3);
					}
				}
